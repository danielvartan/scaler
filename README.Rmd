...
`scaler` is an R package that provides a set of tools to deal with different health measurement scales. The aim of `scaler` is to facilitate the work of researches while also helping with research reproducibility.

Some scales requires a dedicated package. If you're looking for tools to process the Munich ChronoType Questionnaire ([MCTQ](https://doi.org/10.1177/0748730402239679)), please check the [`mctq`](http://gipso.github.io/mctq) package.
...
`scaler` is still at the [experimental](https://lifecycle.r-lib.org/articles/stages.html#experimental) stage of development. That means people can use the package and provide feedback, but it comes with no promises for long term stability.
...
`scaler` is a community project, everyone is welcome to contribute. Take a moment to review our [Guidelines for Contributing](https://gipso.github.io/scaler/CONTRIBUTING.html).
...
To make a donation click on the [Ko-fi](https://ko-fi.com/danielvartan) button above. Please indicate the `scaler` package in your donation message.
